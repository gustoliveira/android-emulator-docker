...
RUN apt-get update 
RUN apt-get install gnupg wget curl gnupg2 git wget unzip gcc-multilib libglu1 -y

# Install java 17
RUN wget https://download.oracle.com/java/17/archive/jdk-17_linux-x64_bin.deb
RUN apt-get install ./jdk-17_linux-x64_bin.deb -y
ENV JAVA_HOME=/usr/lib/jvm/jdk-17/
ENV PATH=/usr/lib/jvm/jdk-17/:bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
...
RUN yes | sdkmanager "emulator" --verbose
